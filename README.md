...
たとえば、コンポーネントは小さく作り(project component)、そのコンポーネントをレイアウトするコンポーネント(container component)を作って配置する。
...
この場合に、Stateは一時的な状態で、[信頼的できる唯一の情報](https://en.wikipedia.org/wiki/Single_source_of_truth)としてはドメインを利用したほうが都合がよいことが多いです。
そのため、Stateはドメインから作成できるようにしておくと、Stateを作り直すことが簡単になります。
...
そのため、基本的にはドメインを信頼できる情報源として、StateはViewのための写像とした方がよいです。
...
CoCが増えるとプロジェクトに参加する人が覚えることが増えるため、できるだけ普通に書けるようにする。
...
覚えることが増えるのでツール側で解消できるなら、そちらのほうが覚えることが少ない。
...
ユーザの入力に対して1F以内に反応を適用しないと体験が良くないケースがある。
